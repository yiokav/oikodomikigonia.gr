...
<div class="ase-page">
  <div class="container">
    ...
    <section class="ase-section" id="cat-{{ $catKey }}">
      <h2 class="ase-category-title">{{ $group['label'] }}</h2>
      <div class="row g-4">
        @foreach($group['items'] as $item)
        <div class="col-sm-6 col-lg-4">
          <article class="ase-product-card">
            <a href="{{ route('ase.show', $item['slug']) }}" class="card-link">
              ...
              <div class="card-body">
                <h3 class="ase-product-card-title">{{ $item['name'] }}</h3>
                @if(!empty($item['short']))
                  <p class="ase-product-card-desc">{{ Str::limit($item['short'], 110) }}</p>
                @else
            ...
            <div class="card-body pt-0">
              <a href="{{ route('ase.show', $item['slug']) }}" class="btn btn-ase">Λεπτομέρειες</a>
            </div>
          </article>
        </div>
        @endforeach
